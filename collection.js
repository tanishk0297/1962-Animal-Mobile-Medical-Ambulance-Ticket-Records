...
const base7 = new Airtable({ apiKey: 'pat9fREdITpFW3UdB.13d5c2b0a2e5a4316b7124d354081bd11ced915241a18dc56a5b913501127ef2' }).base('apppQcGGzTZKVtpmG');
...
async function fetchRecordsFromBase7(selectedCarNumber, startDate, endDate) {
    try {
        const filterFormula = generateFilterFormula(selectedCarNumber, startDate, endDate);
        const records = await base7('Collection').select({
            maxRecords: 10000,
            view: 'Grid view',
            filterByFormula: filterFormula
        }).firstPage();
        ...
        console.error('Error fetching Collection records from base7:', err);
        return []; // Return an empty array in case of an error
    }
}
...
async function fetchRecords() {
    try {
        ...
        const recordsFromBase7 = await fetchRecordsFromBase7(selectedCarNumber, startDate, endDate);

        allRecords = recordsFromBase7.concat(recordsFromBase6,recordsFromBase5,recordsFromBase4, recordsFromBase3,recordsFromBase2,recordsFromBase1); // Concatenate records from all three bases
...
async function deleteRecord(recordId) {
    try {
        await base7('Collection').destroy(recordId);
